...
def move_block(pub_cmd, loop_rate, start_xw_yw_zw, target_xw_yw_zw, vel, accel):
    ...
    error = 0
    print("print the home", home)
    move_arm(pub_cmd, loop_rate, home, 4.0, 4.0)
    time.sleep(0.5)
    
    tmp = lab_invk(start_xw_yw_zw[0], start_xw_yw_zw[1], start_xw_yw_zw[2], 0)
    print("print first theta", tmp)
    move_arm(pub_cmd, loop_rate, tmp, 4.0, 4.0)
    ...
    tmp = lab_invk(target_xw_yw_zw[0], target_xw_yw_zw[1], target_xw_yw_zw[2], 0)
...
def main():
    ...
    target1 = [0.1, -0.1, 0.032]
    target2 = (0.15, -0.1, 0.032)
    ...
    i = 1
    flag = 0
    while(len(xw_yw_G) is not 0):
        flag += 1
        tmpy = xw_yw_G[i][0]
        tmpx = xw_yw_G[i][1]
        tmpz = 0.032
        start = [tmpx, tmpy, tmpz]
        print("start:", start)
        move_block(pub_command, loop_rate, start, target1, vel, accel)
        target1[0]+=0.05
        i-=1
        if flag >= 2:
            break
